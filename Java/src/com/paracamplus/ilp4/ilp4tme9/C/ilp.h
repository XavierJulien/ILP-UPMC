...
extern int nb_call_find_method_global;
extern int nb_call_find_method_line;
extern int nb_call_done;

#define ILP_find_method_inline_cache(res,receiver,method,argc)\
	nb_call_find_method_global++;\
	static ILP_Class c01 = NULL;\
	static ILP_Method m01 = NULL;\
	static int a01 = -1;\
	static ILP_general_function call = NULL;\
	if(receiver->_class == c01 && method == m01 && argc == a01){\
		nb_call_done++;\
		fprintf(stderr, "nombre d'appel hit : %d\n", nb_call_done);\
		fprintf(stderr, "nombre d'appel total : %d\n", nb_call_find_method_global);\
	}else{\
		c01 = receiver->_class;\
		m01 = method;\
		a01 = argc;\
		call = ILP_find_method (receiver,method,argc);\
		fprintf(stderr, "miss");\
		fprintf(stderr, "nombre d'appel total : %d\n", nb_call_find_method_global);\
	}\
	res = call
...
extern ILP_general_function ILP_find_method_global_cache (ILP_Object receiver,
                                             ILP_Method method,
                                             int argc);
extern ILP_general_function ILP_find_method_line_cache (ILP_Object receiver,
                                             ILP_Method method,
                                             int argc);
