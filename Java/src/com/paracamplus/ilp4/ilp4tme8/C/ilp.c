...
{
	ILP_Class c = target->_class;
	while(c->_content.asClass.last_field != NULL){
		ILP_Field f= c->_content.asClass.last_field;
		if(strcmp(f->_content.asField.name,property) == 0){
			return &(target->_content.asInstance.field[f->_content.asField.offset]);
		}
		c->_content.asClass.last_field = c->_content.asClass.last_field->_content.asField.previous_field;
	}
...
ILP_Object
ILP_has_property(ILP_Object target, ILP_Object property)
{
	if(ILP_lookup_property(target,property->_content.asField.name) == NULL) return ILP_FALSE;
	return ILP_TRUE;
}

ILP_Object
ILP_read_property(ILP_Object target, ILP_Object property)
{
	ILP_Object* o = ILP_lookup_property(target,property->_content.asField.name);
	if(o == NULL) return ILP_domain_error("The property does not exist", property);
	return *o;
}
ILP_Object
ILP_write_property(ILP_Object target, ILP_Object property, ILP_Object value)
{
	ILP_Object* o = ILP_lookup_property(target,property->_content.asField.name);
	if(o == NULL) return ILP_domain_error("The property does not exist", property);
	ILP_Object temp = *o;
